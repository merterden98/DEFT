"""Analysis of foldseek alignments: parsing .m8 files and joining EC labels.

This module never shells out to foldseek — it operates on the alignment
DataFrame produced by `foldseek.align` (read via `read_aln`) and the EC
predictions from `loader.predict_ec`. The binary adapter lives in
`foldseek.py`.
"""
