...
def run_foldseek(folder, ec_data, foldseek="foldseek") -> Tuple[Dict[str, SeqRecord.SeqRecord], Dict[str, SeqRecord.SeqRecord], str]:
        ...
        for root,_,files in os.walk(folder):
            for file in files:
                if file.endswith(".cif"):
                    ...
                    os.rename(f"{folder}/{file}", f"{folder}/{uniprot_id}_{ec_data[uniprot_id]}.cif")
    ...
    with contextlib.nullcontext(tempfile.mkdtemp()) as tmpdir:
        ...
        seq_records = SeqIO.to_dict(SeqIO.parse(f"{tmpdir}/{pdb_dir_name}.fasta", "fasta"))
        ...
        seq_records_struct = SeqIO.to_dict(SeqIO.parse(f"{tmpdir}/{pdb_dir_name}_ss.fasta", "fasta"))
        # Update the keys to only have uniprot id 
...
def main_species(args : CreateDatasetSpecies):
    ...
    for uniprot_id in seq_records.keys():
            items.append({"ID": uniprot_id, "Sequence": str(seq_records[uniprot_id].seq), "3DI": str(seq_records_struct[uniprot_id].seq)})
...
def main(args : CreateDataset):
    ...
    if args.ec:
        ...
        for uniprot_id in uniprot_ids:
            if uniprot_id not in ec_data:
                print(f"Uniprot ID {uniprot_id} not found in the EC data", file=sys.stderr)
                sys.exit(1)
    ...
    seq_records, seq_records_struct, traindb_path = run_foldseek(tmp_folder_train, ec_data)
    ...
    for uniprot_id in uniprot_ids:
        try:
            if args.ec and ec_data is not None:
                items.append({"ID": uniprot_id, "Sequence": str(seq_records[uniprot_id].seq), "3DI": str(seq_records_struct[uniprot_id].seq), "EC": ec_data[uniprot_id]})
            else:
                items.append({"ID": uniprot_id, "Sequence": str(seq_records[uniprot_id].seq), "3DI": str(seq_records_struct[uniprot_id].seq)})
        except KeyError:
            missing += 1
    ...
    if args.test_ec and args.test_file and args.mode == "train":
        ...
        tmp_folder_test = query_alphafold.get_pdb_files(uniprot_ids, tmp_folder, f"{name}_test")
